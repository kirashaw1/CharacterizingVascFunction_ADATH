...
## load data table: 
data_table <- read_excel("U:\\Kira\\Projects\\Beth_MixedDiseaseModels\\Data\\2DOIS\\Exp1_2sStim\\2sStim_data4LMM.xlsx")

#data_table <- read_excel("U:\\Kira\\Projects\\Beth_MixedDiseaseModels\\Data\\2DOIS\\Exp3_16sStim\\16sStim_data4LMM.xlsx")
...
data_table$trialGrp[data_table$trialID>5 & data_table$trialID<25]<-'Middle'
data_table$trialGrp[data_table$trialID>=25]<-'Late'
...
<font size="10"> Boxplots and LMM results for 2s stim across disease groups: </font> 
...
variablesofinterest <- c("hbt_W_maxpk","hbt_W_t2p","hbt_W_AUC", "hbr_W_maxpk","hbr_W_t2p","hbr_W_AUC",
                         "hbt_A_maxpk","hbt_A_t2p","hbt_A_AUC", "hbr_A_maxpk","hbr_A_t2p","hbr_A_AUC",
                         "hbt_V_maxpk","hbt_V_t2p","hbt_V_AUC", "hbr_V_maxpk","hbr_V_t2p","hbr_V_AUC")
...
<font size="10"> Boxplots and LMM results for 2s stim across disease groups AND locomotion condition: </font> 
...
<font size="10"> Impact of trial number (1-30) or experimental day (1-4) - scatterplots: </font> 
...
data_table <- data_table %>% filter(expID!="5")
...
for(i in 1:length(variablesofinterest)) { #loop ts metrics of interest
  ...
  print("Group 1, Imaging Day 4:")
  t <- data_table %>% filter(grpID=="AD" & expID=="4")
  print(cor.test(formula = ~trialID+get(variablesofinterest[i]), data=t))
  ...
  print(cor.test(formula = ~trialID+get(variablesofinterest[i]), data=t))
  print("Group 2, Imaging Day 4:")
  t <- data_table %>% filter(grpID=="WT" & expID=="4")
  print(cor.test(formula = ~trialID+get(variablesofinterest[i]), data=t))
  
  print("Group 3, Imaging Day 1:")
  t <- data_table %>% filter(grpID=="ATH" & expID=="1")
  print(cor.test(formula = ~trialID+get(variablesofinterest[i]), data=t))
  print("Group 3, Imaging Day 4:")
  t <- data_table %>% filter(grpID=="ATH" & expID=="4")
  print(cor.test(formula = ~trialID+get(variablesofinterest[i]), data=t))
  
  print("Group 4, Imaging Day 1:")
  t <- data_table %>% filter(grpID=="MIX" & expID=="1")
  print(cor.test(formula = ~trialID+get(variablesofinterest[i]), data=t))
  print("Group 4, Imaging Day 4:")
  t <- data_table %>% filter(grpID=="MIX" & expID=="4")
  print(cor.test(formula = ~trialID+get(variablesofinterest[i]), data=t))
...
<font size="10"> Impact of trial number (1-30) - boxplots and LMMs: </font> 
...
<font size="10"> Impact of experimental day (1-4)- boxplots and LMMs: </font> 
